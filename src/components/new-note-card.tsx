          ...
          <form className='flex-1 flex flex-col'>
            <div className='flex flex-1 flex-col gap-3 p-5'>
              ...
              {shouldShowOnboarding ? (
                <p className='text-small leading-6 text-slate-400'>Comece <button type='button' onClick={handleStartRecording} className='font-medium text-lime-400 hover:underline'>gravando uma nota</button> em áudio ou se preferir <button type='button' onClick={handleStartEditor} className='font-medium text-lime-400 hover:underline'>utilize apenas texto</button> .</p>
              ) : (
                <textarea autoFocus className='text-sm leading-6 bg-transparent resize-none felx-1 outline-none' onChange={handleContentChanged} value={content}/>
              )}
            </div>
